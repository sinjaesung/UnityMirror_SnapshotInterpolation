...
public class ServerCube : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            Debug.Log("ServerCube Time.time >= lastSendTime + sendInterval");
    ...
    void Flush()
    {
        ...
        for (int i = 0; i < queue.Count; ++i)
        {
            ...
            Debug.Log($"ServerCube Flush Queue Index>{i}=> deliveryTime:{deliveryTime},snap:{snap}");
            ...
            if (NetworkTime.localTime >= deliveryTime)
            {
                ...
                queue.RemoveAt(i);
                Debug.Log($"ServerCube client.OnMessage(snap) queue.RemoveAt {i}");
                --i;
                Debug.Log($"ServerCube client.OnMessage(snap) queue --i {i}");
            }
        }
    }
}
